...
def test_split_data():
    ...
    df = pd.DataFrame(
        ...
        columns=["var_%d" % i for i in range(0, 10)]
        )
    target  = "churn"
    df[target] = [np.random.randint(0, 2) for _ in range(20)]
 
    X_train, X_test, y_train, y_test = preprocessing.split_data(df, target, 0.2)
    ...
    X_train, X_test, y_train, y_test = preprocessing.split_data(df, target, 0.1)
